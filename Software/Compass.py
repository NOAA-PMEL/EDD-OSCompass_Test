...
for i in range(8):
	if(compass[i].installed):
		print("Compass %d: SN# %d" % (i, compass[i].serialNumber)

pmel_owner= input("Which group owns these compasses: GTMBA, OCS, EDD, TEST?" ) 
for i in range(8):
	if(compass[i].installed):
		calibration.set_owner(compass[i], pmel_owner)
		
test_date= input("What is today's date? mmddyyyy: ")
for i in range(8):
	if(compass[i].installed):
		calibration.set_testDate(compass[i], test_date)
